...
- **🔒 SECURITY — preview now flags off-host navigate and credential-shaped fills.** `web/src/state/compileFlags.ts` exposes pure detectors: `isExternalUrl` flags off-localhost `navigate` blocks; `looksLikeCredential` flags `fill` blocks whose target description or value matches credential / SSN / credit-card patterns. The preview shows a "Review carefully — N blocks flagged" banner when any flag fires, and per-block badges with the reason (tooltip + visible). Regression: `web/src/__tests__/compileFlags.test.ts`. Remaining gaps: no diff against existing blocks on Append; no per-block accept/reject. Document those as the next hardening pass.
...
- **Resolved — `confirm()` for Replace migrated to UiPrompts.** Now uses `useUiPrompts().confirm({ destructive })` consistent with the rest of the app.
