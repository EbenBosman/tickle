...
- **⚠️ Drift — `MODELS` allowlist is `keep-in-sync`.** The server's source of truth is `server/src/domain/models.ts` (with the typed `isValidModel` guard); the client's `MODELS` array carries the richer label/cost metadata for display and is marked `keep-in-sync`. A cross-workspace shared `domain/` would unify them; deferred until tsconfig project references or a `shared/` workspace land.
