...
- **Resolved — `VALID_MODELS` server-side hoisted.** Now at `server/src/domain/models.ts` with a typed `isValidModel` narrowing guard; `routes/settings.ts` imports the guard. The web `SettingsPage` keeps a richer `MODELS` array (label + cost) for presentation and is marked `keep-in-sync`. A cross-workspace shared `domain/` would unify them; deferred until tsconfig project references or a `shared/` workspace land. Regression: `__tests__/models.test.ts`.
...
- **Resolved — `?limit=` and `?offset=` clamps.** `/api/lessons` now coerces to finite positive numbers; non-numeric / negative / zero `limit` falls back to the default 50; `limit > 200` clamps to 200. Regression: `routes/__tests__/settings.test.ts`.
