...
| §6 ⚠️ `finish` schema vs `finish_step` interception drift                    | —         | —         | TODO(test) |
...
- ⚠️ **`finish` vs `finish_step` schema/interception drift.** `tools.ts::toolDefs` declares the LLM-facing tool as `finish(answer)`. `agent.ts::toolsForAiBlock` *appends* a second tool `finish_step(success, output?, note?)` (lines 1513–1534), and the agent loop intercepts only `finish_step`. The model therefore sees BOTH tools and may pick `finish` — which `executeTool` will dispatch to (returning `{ ok: true, text: <answer> }`) and the loop will continue rather than terminate, falling into stall detection or the per-block step-limit failure. Fix: pick one name and remove the other from one site or the other. Recommended: drop the `finish` entry from `toolDefs` and keep `finish_step` (the agent.ts contract). Cross-spec with `tools.md` §6.
- ⚠️ **`block_end` is emitted twice on a successful rescue.** `executeBlocks` emits `block_end` with `status: "failed"` after the local attempt (lines `323–339`), then — if rescue succeeds — emits a second `block_end` with `status: "done"` (lines `358–369`). The UI's `RunView` last-write-wins is not guaranteed by the contract; the persistent `steps` table now contains two `block_end` rows for one block. Fix: skip the first `block_end` emission when rescue is going to be attempted, OR mark the first as a tentative `block_end_attempt` kind, OR drop the duplicate persistence and only emit a single rolled-up event after the rescue decision.
...
- ⚠️ **CLAUDE.md "Sub-agent loop" docs the wrong tool name.** It says `finish_step(success, output?, note?)` matches the running code — but the schema in `tools.ts` says `finish`. The handbook is consistent with `agent.ts`, inconsistent with `tools.ts`. Cross-spec consequence of the same drift.
