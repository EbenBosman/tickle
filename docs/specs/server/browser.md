...
- **I3 — Profile path is `server/data/profile/`.** Resolved via `import.meta.url` (anchored at `server/src/paths/storage.ts::PROFILE_DIR`); `TICKLE_PROFILE_DIR` overrides (absolute path passes through; relative resolves against `server/`). Auto-created with `mkdirSync(..., { recursive: true })` on module import.
...
- **Resolved — profile and screenshot paths anchored.** `PROFILE_DIR` / `SHOTS_DIR` now live in `server/src/paths/storage.ts`, anchored via `import.meta.url`. `TICKLE_PROFILE_DIR` and `TICKLE_SHOTS_DIR` env vars override (absolute pass-through, relative resolves against `server/`). `paths.ts::SCREENSHOTS_DIR` re-exports `SHOTS_DIR` so `routes/runs.ts` and the browser module agree on the location. Regression: `__tests__/browser.paths.test.ts`.
