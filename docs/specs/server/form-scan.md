...
- ❓ **`SELECTOR` includes `[role="combobox"]`** but the type classifier has no `combobox` branch — it falls into `"other"`. Comboboxes are common (especially in design-system libraries) and would currently be tagged but unfillable. Either map `combobox` → `select` or drop it from SELECTOR.
