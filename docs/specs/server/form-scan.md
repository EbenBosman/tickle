...
- **Resolved — visibility check unified.** `server/src/visibility.ts` exports `isVisuallyHidden(style, rect?)` with `parseFloat(opacity) === 0`. `formScan.ts` and `loginDetect.ts` keep their inline `page.evaluate` copies (mirrored logic, marked `keep-in-sync: visibility.ts`) so the same `0.0` / `0.00` opacity case is caught in both. Regression: `__tests__/loginDetect.test.ts`.
