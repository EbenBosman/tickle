...
> **Non-obvious why:**
> - **Pause is not cancel.** `pause` must NOT abort the in-flight LLM request — that is `cancel.ts`'s job (which calls `client.abort()`). Pause only blocks at the *next* safe boundary in the executor; the current tool call / chat response runs to completion first.
...
- `awaitIfPaused` resolves immediately (already-resolved Promise) for unknown or not-paused runs; otherwise resolves on the next `resume` *or* `clear` for that run.
...
- **Waiters fire exactly once per pause cycle.** A `Promise` returned by `awaitIfPaused` resolves the first time `resume` or `clear` is called for that run; subsequent pauses produce *new* promises with their own waiter list.
