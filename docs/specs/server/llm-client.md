...
> Path: `server/src/llm.ts` (+ `chatWithRetry` currently colocated in `server/src/agent.ts:31-74`) · Layer: `infrastructure/llm/` (post-refactor target — `infrastructure/llm/openaiCompatClient.ts`, `infrastructure/llm/anthropicClient.ts`, `infrastructure/llm/chatWithRetry.ts`) · Spec owner: `agent.ts` (multi-turn sub-agent loops + rescue), `routes/compile.ts` (stateless one-shot)
...
| `newAnthropicClient` | function | `(model: string) => LlmClient` — always Anthropic; for the rescue path, independent of primary provider   | stable    |
...
- **⚠️ Drift — `newAnthropicClient(model)` ignores its argument.** The signature accepts `model: string` but the body never reads it; the model name is taken from DB settings at the call site. Either remove the parameter or use it. Confusing as-is.
