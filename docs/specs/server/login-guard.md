...
- **Resolved — CLAUDE.md SSO list.** CLAUDE.md now lists `X / twitter.com` alongside the other SSO providers, matching `KNOWN_LOGIN_PATHS`.
...
- **Resolved — visibility check.** `loginDetect.ts`'s inline `page.evaluate` copy now uses `parseFloat(opacity) === 0`, mirrored from `server/src/visibility.ts::isVisuallyHidden` and marked `keep-in-sync`. Catches `0`, `0.0`, `0.00`, etc. Regression: `__tests__/loginDetect.test.ts`. `offsetParent` and occlusion / viewport-scroll checks remain out of scope.
