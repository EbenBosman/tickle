...
- **I3 — Visibility is a hard gate.** An element is included only if (a) `getBoundingClientRect()` has non-zero width and height, (b) computed `display !== "none"`, `visibility !== "hidden"`, and `opacity > 0`, (c) no ancestor has `display:none` or `visibility:hidden`, and (d) no ancestor has `aria-hidden="true"` (`el.closest("[aria-hidden='true']")`). Falsifiable: render a `display:none` button — it must not appear.
...
- **I9 — Single-pass tagging is destructive but idempotent.** Each call clears stale `[data-tickle-id]` attributes at the top of the `page.evaluate` block before retagging. The `act` tool in `tools.ts` resolves IDs strictly within one snapshot→act window.
...
- **Resolved — stale `data-tickle-id` cleanup.** `snapshot.ts` and `formScan.ts` now strip stale `[data-tickle-id]` attributes at the top of each `page.evaluate` block before retagging. Regression: `__tests__/snapshot.test.ts`.
- **Resolved — `aria-hidden="true"` filter.** `isVisible` now rejects elements with an `aria-hidden="true"` ancestor via `el.closest("[aria-hidden='true']")`. `aria-hidden="false"` does NOT hide. Regression: `__tests__/snapshot.test.ts`.
